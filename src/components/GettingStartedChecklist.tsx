...
export default function GettingStartedChecklist({ state }: GettingStartedChecklistProps) {
  ...
  if (state.kind === 'parent') {
    return (
      <section className="mt-6 rounded-3xl border border-amber-100 bg-amber-50 p-4 shadow-[0_14px_35px_rgba(15,23,42,0.05)] sm:p-5">
...
function SecondaryLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="inline-flex rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-800 shadow-sm hover:bg-slate-50">
      {children}
    </Link>
  )
}
