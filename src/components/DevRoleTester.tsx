...
async function switchRoleTesterUser(formData: FormData) {
  ...
  redirect('/super-admin/dev-tools/role-tester')
}

export default async function DevRoleTester() {
  if (!isRoleTesterEnabled()) {
    return (
      <main className="mx-auto w-full max-w-4xl px-4 py-6 sm:p-6">
        <PageHeader title="Dev Role Tester disabled" description="Set ENABLE_ROLE_TESTER=true to enable this local development tool." />
      </main>
    )
  }
  ...
  if (isClerkEnabled()) {
    return (
      <main className="mx-auto w-full max-w-4xl px-4 py-6 sm:p-6">
        <PageHeader title="Dev Role Tester unavailable" description="Dev Role Tester is unavailable while Clerk authentication is enabled." />
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-6 sm:p-6">
      <PageHeader
        ...
        description="Act as seeded local test users to test server-side permissions and role-specific journeys."
      />

      <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950 shadow-sm">
        <h2 className="font-bold">Development only. Switches between seeded local test users. This does not change real user roles.</h2>
        <p className="mt-1">Switching changes the current local test user cookie. Existing server-side permission checks still apply.</p>
      </section>

      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <p className="text-sm font-bold uppercase tracking-wide text-slate-500">Current test user</p>
