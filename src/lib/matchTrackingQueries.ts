...
export async function getPreviousTrackingTasksForCopy(userId: string, destinationMatchDayId: string) {
  ...
  return prisma.matchTrackingTask.findMany({
    ...
    include: { matchDay: true, events: { include: { matchDayEventType: { include: { eventDefinition: true } } } }, patterns: { include: { pattern: { select: { name: true } } }, orderBy: { displayOrder: 'asc' } }, clubDefinitions: { include: { clubTrackingDefinition: { select: { name: true } } }, orderBy: { displayOrder: 'asc' } } },
    orderBy: { createdAt: 'desc' },
    take: 20,
  })
}
